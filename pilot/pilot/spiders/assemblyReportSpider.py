...
class assemblyReportSpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        attendance_rate = extract_attendance_rate(hxs)
        proposal = extract_proposal(hxs)
        print(id)
        print(attendance_rate)
        print(proposal)
        yield items.AssemblyMemberItem(assembly_id=id,attendance_rate=attendance_rate,proposal=proposal)
